...
const ApiRouter = function (webserver) {
    ...
    router.get("/current_zones", async (req, res) => {
        const zones = [...configuration.getZones().values()];
        let robotPos = webserver.map.parsedData.robot;
        let rX = robotPos[0];
        let rY = robotPos[1];
        let zoneMatches = [];
        zones.forEach((zone, index) => {
            ...
            zone.areas.forEach((area) => {
                if (rX >= area[0] && rX <= area[2] && rY >= area[1] && rY <= area[3]) {
                    //zoneMatches[index] = zone;
                    zoneMatches.push({
                        index: index,
                        id: zone.id,
                        name: zone.name
    ...
    router.get("/fw_version", async (req, res) => {
        ...
        if (model.isEmbedded()) {
            try {
                ...
                const matchedContent = data.toString().match(/^ROBOROCK_VERSION=[\d.]+_(?<fwVersion>\d+)$/m);

                if (matchedContent && matchedContent.groups) {
                    firmwareVersion = matchedContent.groups.fwVersion;
                }
            } catch (err) {
                //intentional
            }
